...
from dataset_generator_good import DataGenerator as Generator_iaa
...
def create_train_val_datasets(frames):
    ...
    train_generator = Generator_iaa(input_channels, patch_size, training_frames, frames, annotation_channels,
...
def create_callbacks(model_path):
    ...
    class CustomMeta(Callback):
		...
		"dice_coef": float(f"{logs['dice_coef']:.4f}"),
		...
		"last_precision": float(f"{logs['precision']:.4f}"),
		...
		"true_positives" : float(f"{logs['true_positives']:.4f}"),
		"true_negatives" :float(f"{logs['true_negatives']:.4f}"),
		"false_positives" :float(f"{logs['false_positives']:.4f}"),
		"false_negatives" :float(f"{logs['false_negatives']:.4f}"),
            }
    ...
    return [checkpoint, tensorboard, CustomMeta()]#, es]
...
def train_model(conf):
    ...
    if config.continue_model_path is not None:
        ...
        model = tf.keras.models.load_model(config.continue_model_path,
                                           custom_objects={'tversky': get_loss('tversky', config.tversky_alphabeta),
                                                           ...
                                                           'sensitivity': sensitivity, 'precision': precision}, compile=False)
    ...
    #model.make_test_function(force=False)
    ...
    results = model.evaluate(test_generator, batch_size=1, steps=8)#, batch_size=30, steps=1) #, max_queue_size=22)
    #results2 = model.test_on_batch(test_generator, sample_weight=None, reset_metrics=True, return_dict=False)
    print(model.metrics_names)
    print(results)
    #print(results2)
   
    name_csv=f"{config.model_name}"
    df=pandas.DataFrame(results,index=model.metrics_names,columns=[name_csv])
    
    #if not os.path.exists(f"{config.test_dir}tests_stat.csv") :
    df.to_csv(f"{config.test_dir}{config.model_name}.csv",decimal=',')
    #else :
        #df_origin = pandas.read_csv(f"{config.test_dir}tests_stat.csv") # Read CSV file
        #df_origin[name_csv] = results                                # Append list as new variable
        #df_origin.to_csv(f"{config.test_dir}tests_stat.csv",decimal=',')
