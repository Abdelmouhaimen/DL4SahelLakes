...
from core.losses import accuracy, dice_coef, dice_loss, specificity, sensitivity, get_loss, precision, true_positives, true_negatives, false_positives, false_negatives, f1score
...
def load_model(config):
    ...
    model = tf.keras.models.load_model(config.trained_model_path,
                                       custom_objects={'tversky': get_loss('tversky', config.tversky_alphabeta),
                                                       ...
                                                       'sensitivity': sensitivity, 'precision': precision,'true_positives':true_positives,'true_negatives':true_negatives,
							'false_positives':false_positives, 'false_negatives':false_negatives, 'f1score':f1score},
                                       compile=False)
...
def predict_using_model(model, batch, batch_pos, mask, operator):
    ...
    for i in range(len(batch_pos)):
        ...
        # Replace nodata values in the prediction with the nodata value
        p[np.isnan(tm[i,:,:,1])] = 0
...
def predict_image(params):
    ...
    pos = min(int(multiprocessing.current_process().name[-1]), config.prediction_workers) + 1  # progress bar position
