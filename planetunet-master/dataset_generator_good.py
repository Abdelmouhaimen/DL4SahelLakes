...
def imageAugmentationWithIAA():
	...
	#iaa.Crop(percent=(0, 0.1)),
        iaa.ScaleX((0.5, 1.5)),
        iaa.ScaleY((0.5, 1.5)),
        iaa.Rotate((-45, 45)), # essayer pleins de rotate
        iaa.TranslateX(percent=(-0.1, 0.1)),
        iaa.TranslateY(percent=(-0.1, 0.1)),
        #sometimes(iaa.ScaleX((0.5, 1.5))),
        #sometimes(iaa.ScaleY((0.5, 1.5))),
        #sometimes(iaa.Rotate((-45, 45))), # essayer pleins de rotate
        #sometimes(iaa.TranslateX(percent=(-0.1, 0.1))),
        #sometimes(iaa.TranslateY(percent=(-0.1, 0.1))),
	#sometimes(iaa.Resize({"height": 32, "width": "keep-aspect-ratio"})), # ne marche pas

        # # Gaussian blur and gamma contrast
        # sometimes(iaa.GaussianBlur(sigma=(0, 0.3)), 0.3), 
        # sometimes(iaa.GammaContrast(gamma=0.5, per_channel=True), 0.3),# play with brightness
        ...
        sometimes(iaa.PiecewiseAffine(0.05), 0.3), # distortion
